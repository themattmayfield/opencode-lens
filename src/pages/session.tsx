	...
	const FileVisual = (props: { file: LocalFile }): JSX.Element => {
		return (
			<div class="flex items-center gap-x-1.5">
				<FileIcon
					node={props.file}
					class="grayscale-100 group-data-[selected]/tab:grayscale-0"
				/>
				<span
					classList={{
						"text-14-medium": true,
	...
	return (
		<div class="relative bg-background-base size-full overflow-x-hidden">
			...
			>
				...
				<ConstrainDragYAxis />
				<Tabs
					value={session.layout.tabs.active ?? "chat"}
					onChange={session.layout.openTab}
				>
							...
							<Tabs.Trigger value="chat">
								<div class="flex gap-x-[17px] items-center">
									...
									>
										...
										<div class="text-14-regular text-text-weak text-left w-7">
								...
								<Tabs.Trigger
									...
									closeButton={
										<IconButton
											...
											size="normal"
											variant="ghost"
											onClick={local.layout.review.pane}
										/>
									}
								>
								...
								<For each={session.layout.tabs.opened ?? []}>
									{(tab) => (
										<SortableTab
											...
											onTabClose={session.layout.closeTab}
										/>
									)}
								</For>
							</SortableProvider>
					...
					<Tabs.Content
						value="chat"
						class="@container select-text flex flex-col flex-1 min-h-0 overflow-y-hidden"
					>
						<div
							classList={{
										...
										<div
											classList={{
												...
												"flex items-start justify-start":
													local.layout.review.state() === "pane",
											}}
										>
											<Show when={session.messages.user().length > 1}>
												<ul
													...
													classList={{
														...
														"absolute right-full w-60 mt-3 @7xl:gap-2 @7xl:mt-1":
															local.layout.review.state() === "tab",
													...
													<For each={session.messages.user()}>
														{(message) => {
															...
															const error = createMemo(
																() =>
																	assistantMessages().find((m) => m?.error)
																		?.error,
															);
															...
															const handleClick = () =>
																session.messages.setActive(message.id);
											...
											<div
												...
												class="grow size-full min-w-0 overflow-y-auto no-scrollbar"
											>
												<For each={session.messages.user()}>
													{(message) => {
														const isActive = createMemo(
															() =>
																session.messages.active()?.id === message.id,
														);
														const [titled, setTitled] = createSignal(
															!!message.summary?.title,
														);
														const assistantMessages = createMemo(() => {
															if (!session.id) return [];
														...
														createEffect(() => {
															...
															// Also consider complete if we have assistant messages and they're done
															const hasResponse =
										...
										<div class="size-full flex flex-col pb-45 justify-end items-start gap-4 flex-[1_0_0] self-stretch">
											...
											<div class="flex justify-center items-center gap-3">
												...
												<div class="text-12-medium text-text-weak">
													...
													<span class="text-text-strong">
														{DateTime.fromMillis(
															sync.data.project.time.created,
														).toRelative()}
													</span>
												</div>
											</div>
					...
					<Show
						when={
							local.layout.review.state() === "tab" && session.diffs().length
						}
					>
						<Tabs.Content
							value="review"
							class="select-text flex flex-col h-full overflow-hidden"
						>
							<div
								classList={{
							...
							{(path) => (
								<li>
									...
									>
										...
										/>
										<span class="text-xs text-text whitespace-nowrap">
											{getFilename(path)}
										</span>
			...
			<Show when={store.fileSelectOpen}>
				<SelectDialog
					...
					onSelect={(x) =>
						x ? session.layout.openTab("file://" + x) : undefined
					}
				>
					{(i) => (
						<div
