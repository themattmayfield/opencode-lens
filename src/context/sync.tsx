...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
	...
	init: () => {
		...
		const load = {
			project: () =>
				sdk.client.project.current().then((x) => setStore("project", x.data!)),
			provider: () =>
				sdk.client.config
					.providers()
			...
			changes: () =>
				sdk.client.file.status().then((x) => setStore("changes", x.data!)),
			node: () =>
				sdk.client.file
					.list({ query: { path: "/" } })
		...
		Promise.all(Object.values(load).map((p) => p()))
			...
			.catch((error) => {
				...
				console.error("Make sure the OpenCode server is running:");
				// console.error("  cd packages/opencode && bun dev")
			});

		const sanitizer = createMemo(
			() => new RegExp(`${store.path.directory}/`, "g"),
		);
		const sanitize = (text: string) => text.replace(sanitizer(), "");
		const absolute = (path: string) =>
			(store.path.directory + "/" + path).replace("//", "/");
				...
				if (
					...
					for (const key in part.state.metadata) {
						if (typeof part.state.metadata[key] === "string") {
							part.state.metadata[key] = sanitize(
								part.state.metadata[key] as string,
							);
						}
					}
