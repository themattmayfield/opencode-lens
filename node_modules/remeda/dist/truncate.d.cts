import { IsStringLiteral, NonNegativeInteger, IsEqual, And } from 'type-fest';
 ...
 */
type StringLength<S extends string, Characters extends ReadonlyArray<string> = []> = If<IsStringLiteral<S>, S extends `${infer Character}${infer Rest}` ? StringLength<Rest, [...Characters, Character]> : Characters["length"], number>;
...
type Truncate<S extends string, N extends number, Options extends TruncateOptions> = TruncateWithOptions<S, N, Options extends Pick<Required<TruncateOptions>, "omission"> ? Options["omission"] : typeof DEFAULT_OMISSION, Options extends Pick<Required<TruncateOptions>, "separator"> ? Options["separator"] : undefined>;
type TruncateWithOptions<S extends string, N extends number, Omission extends string, Separator extends string | RegExp | undefined> = N extends unknown ? If<IsEqual<N, 0>, "", Omission extends unknown ? If<IsStringLiteral<Omission>, If<IsEqual<ClampedIntegerSubtract<N, StringLength<Omission>>, 0>, TruncateLiterals<Omission, N, "">, If<And<IsStringLiteral<S>, IsEqual<Separator, undefined>>, TruncateLiterals<S, N, Omission>, string>>, string> : never> : never;
/**
 * This is the actual implementation of the truncation logic. It assumes all
 * its params are literals and valid.
 */
type TruncateLiterals<S extends string, N extends number, Omission extends string, Iteration extends ReadonlyArray<unknown> = []> = S extends `${infer Character}${infer Rest}` ? Iteration["length"] extends ClampedIntegerSubtract<N, StringLength<Omission>> ? If<IsLongerThan<S, Omission>, Omission, S> : `${Character}${TruncateLiterals<Rest, N, Omission, [...Iteration, unknown]>}` : "";
/**
 ...
 * @category String
 */
declare function truncate<S extends string, N extends number, const Options extends TruncateOptions>(data: S, n: NonNegativeInteger<N>, options?: Options): Truncate<S, N, Options>;
declare function truncate(data: string, n: number, options?: TruncateOptions): string;
/**
 * Truncates strings longer than `n`, appending an `omission` marker to them
 * (which defaults to '...'); shorter strings are returned as-is. The total
 * length of the output will never exceed `n` (in the rare case where the
 ...
 * @category String
 */
declare function truncate<N extends number, const Options extends TruncateOptions>(n: NonNegativeInteger<N>, options?: Options): <S extends string>(data: S) => Truncate<S, N, Options>;
declare function truncate(n: number, options?: TruncateOptions): (data: string) => string;
