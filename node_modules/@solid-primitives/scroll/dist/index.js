...
const FALLBACK_SCROLL_POSITION = { x: 0, y: 0 };
/**
 * Get an `{ x: number, y: number }` object of element/window scroll position.
 */
export function getScrollPosition(target) {
    if (isServer || !target) {
 ...
 * });
 */
export function createScrollPosition(target) {
    if (isServer) {
        return FALLBACK_SCROLL_POSITION;
    }
    target = target || window;
    const isFn = typeof target === "function", isHydrating = sharedConfig.context, getTargetPos = isFn
 ...
 * })
 */
export const useWindowScrollPosition = /*#__PURE__*/ createHydratableSingletonRoot(() => createScrollPosition(isServer ? () => undefined : window));
